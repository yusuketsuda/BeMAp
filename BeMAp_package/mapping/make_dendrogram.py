...
def make_dendrogram(pd_dfx, tem_store):
    #各プラスミドをnumberingした結果を読み込む
    du = pd.DataFrame(pd_dfx.values,index=pd_dfx.index)
    du_sort = du.sort_values(len(pd_dfx.columns)-1)
    
    # 後ろから合成していく
    # uniqueな数字をとりだし、一致するものをdictにして、dictのkeyに複数のaccesssion nameを投入していく。繰り返す。
    ...
    for i in reversed(range(1,len(pd_dfx.columns))):
        ...
        for j in da[i].unique():
            if len(da[da[i] == j].index) == 1:
                div_dict_da[str(list(da[da[i] == j].index)).replace("\\","")] = da.loc[da[i] == j,:i-1].values[0] # \がついてくるので抜いておく（処理速度が速くなった）
            else:
    ...
    for i in all_index:
        new_index.append(i.translate(str.maketrans({'[':'(',']':')','\'':'','\"':''}))) # newick形式に調整
    newick_all = str(new_index).translate(str.maketrans({'[':'(',']':')','\'':''})) # newick形式に調整
