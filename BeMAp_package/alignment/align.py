...
def align_target_centered_filled(target_centered):
    gene = [x for x in np.unique(target_centered) if np.isnan(x) == False][1:]
    ...
    while len(du)<len(amr):
        ...
        after_df = df[k]
        if k != 1:
            if (list(du[k-1][0].index) != []) and (len(pre_df) == len(after_df)):
                if check_others_filled(du[k-1][0].T):
                    k += 1
                else:
                    align_target_centered(du[k-1][0])
                    break
            else:
                k += 1
        else:
